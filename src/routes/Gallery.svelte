	...
	<div class="yearFilterButtonGroup">
		<div class="yearButton">
			<Button
				outline
				color="alternative"
				on:click={() => filterImages('all')}
				on:keypress={() => filterImages('all')}
			>
				Alle
			</Button>
		</div>
		{#each years as year}
			<div class="yearButton">
				<Button
					outline
					color="alternative"
					on:click={() => filterImages(year)}
					on:keypress={() => filterImages(year)}
				>
					{year}
				</Button>
			</div>
	...
	>
		...
		>
			{#if selectedImageMetadata.title}
				<h1 class="modalHeader">{selectedImageMetadata.title}</h1>
			{/if}
